...
struct Group {
  ...
  void choose_target(std::vector<Group*>& potential_victims) {
    target = nullptr;

    int max_damage = 0;
    for (Group* potential_victim : potential_victims) {
      if (potential_victim->targetted_by != nullptr || (potential_victim->immune_to & type))
        continue;

      int damage = effective_power();
      if (potential_victim->weak_to & type)
        damage *= 2;
      ...
      if (potential_victim->effective_power() != target->effective_power()) {
        if (potential_victim->effective_power() > target->effective_power())
          target = potential_victim;
        continue;
      }

      if (potential_victim->initiative > target->initiative) {
        target = potential_victim;
      }
    }

    if (target != nullptr)
      target->targetted_by = this;
  }
};
...
void fight(std::vector<Group*>& immune_system, std::vector<Group*>& infection)
{
  ...
  for (Group* attacker : order) {
    ...
    attacker->choose_target(potential_victims);
  }
...
std::vector<Group*> copy_team(const std::vector<Group*>& team)
{
  std::vector<Group*> copy(team.size(), nullptr);
  for (size_t i = 0, endi = team.size(); i < endi; i++) {
    copy[i] = new Group();
    *copy[i] = *team[i];
  }
  return copy;
...
int count_team(const std::vector<Group*>& team)
{
  int result = 0;
  for (const Group* g : team) {
    result += std::max(g->count, 0);
  }
  return result;
}


int fight_until_end(const std::vector<Group*>& orig_immune_system, const std::vector<Group*>& orig_infection, int boost)
{
  std::vector<Group*> immune_system = copy_team(orig_immune_system);
  std::vector<Group*> infection = copy_team(orig_infection);
  ...
  // Fight
  int old_immune_left = count_team(immune_system);
  int old_infection_left = count_team(infection);
  while (true) {
    fight(immune_system, infection);

    int immune_left = count_team(immune_system);
    int infection_left = count_team(infection);
    if (immune_left == 0 || infection_left == 0)
      return immune_left - infection_left;
    if (immune_left == old_immune_left && infection_left == old_infection_left)
      return 0;
    old_immune_left = immune_left;
    old_infection_left = infection_left;
  }
}
...
int main(int argc, char** argv)
{
  ...
  int boost = 0, result = 0;
  for (; result <= 0; boost++)
    result = fight_until_end(immune_system, infection, boost);
  --boost;

  printf("Immune system wins at boost = %d with %d units remaining\n", boost, result);
}
